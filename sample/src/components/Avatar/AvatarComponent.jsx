...
export const AvatarComponent = ({
  ...
  return <div className="tc avatar placeholder relative">
    {
      img !== undefined
        ?
        <img
          src={img}
          className="profile-pic br-100 dib" alt="avatar" />
        :
        <div className="profile-pic br-100 dib bp-bg-sky flex items-center justify-center">
          <img
            src={tenantIcon}
            className="h2 w2" alt="tenant" />
        </div>
    }
    {
      canEdit &&
      <>
        ...
        <input className="dn" type="file" name="profile-pic" id="profile-pic" accept="image/*" onChange={(e) => updateImage(e.target.files[0])} />
      </>
    }
    {
      isLoading &&
